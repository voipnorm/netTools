  ...
  crud.findSpace(spaceId, function(err,spDetails){
    if(err){
      ...
      crud.createSpace(spaceId, function(data){
        ...
        bot.say({markdown:"**Welcome to NetTools Bot.**<br> I am here to help with network related tasks like DNS lookups, Website monitoring and port scans."+
        " Use the **/commands** command to see what I can do.<br> "});
        return;
      });
    }  
    if(spDetails.spaceActive === "false"){
      log.info("flintConfig.spawn : Room already exsisting. No room created.");
      spDetails.updateActive("true");
      spDetails.updateSetup("complete");
      spDetails.writeToFile();
      log.info("flintConfig.spawn : Room has been reactivated");
      
      return;
    }
    if(spDetails.spaceActive === 'true') {
      spDetails.updateSetup("complete");
      return log.info("flintConfig.spawn: Room already active, no room created");
...
flint.on('despawn', function(bot){
  ...
  crud.findSpace(spaceId, function(err,spDetails){
    ...
    spDetails.updateActive("false");
    spDetails.writeToFile();
    spDetails.cancelSchedule();
    spDetails.on('cancel', function(){
                   spDetails.reloadSchedule(); 
                });
  });
});
